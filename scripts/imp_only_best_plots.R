...
print("Entering the R script: imp_only_plot_snakemake.R")
...
plink_dir <- snakemake@params[[2]]
prsice_dir <- snakemake@params[[3]]
lasso_dir <- snakemake@params[[4]]

target_prefix <- snakemake@params[[5]]
external_prefix <- snakemake@params[[6]]
pval_thr <- snakemake@params[[7]] #p-values used in PLINK and PRSice
...
write.table(best_thr.all, snakemake@output[[4]], col.names = TRUE, row.names = FALSE, quote = FALSE)
